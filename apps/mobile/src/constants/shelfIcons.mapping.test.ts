...
describe("canonical shelf icon mapping", () => {
  it("maps default shelf IDs to logical keys and filenames", () => {
    ...
    expect(DEFAULT_SHELF_ICON_FILE.want_to_read).toBe("currently-reading.png");
    expect(DEFAULT_SHELF_ICON_KEY.currently_reading).toBe("open_book");
    expect(DEFAULT_SHELF_ICON_FILE.currently_reading).toBe("did-not-finish.png");
    expect(DEFAULT_SHELF_ICON_KEY.dnf).toBe("closed_book");
    expect(DEFAULT_SHELF_ICON_FILE.dnf).toBe("want-to-read.png");
  ...
  it("validates emoji graphemes and the Custom Collections heading", () => {
    expect(isValidShelfEmoji("❤️")).toBe(true);
    expect("❤️".length).not.toBe(1);
    expect(parseCustomShelfIconSelection({ type: "emoji", value: "📚" }).ok).toBe(true);
    expect(parseCustomShelfIconSelection({ type: "emoji", value: "/x.png" }).ok).toBe(false);
