...
import { mapSubjectsToGenreIds } from "../../../../../packages/utils/challengeGenres";
import {
...
import { challengeCanShareToFeed } from "../../../../../packages/utils/challengeVisibility";
import { formatCommunityMilestone } from "../../../../../packages/utils/challengeDisplay";
import { evaluateAndAwardBadges } from "./ChallengeBadgeService";
import { notifyChallengeEvent } from "./challengeNotifications";
...
export async function evaluateQualifyingEventForChallenges(input: {
  ...
  const db = input.supabase ?? defaultClient;
  ...
  const clubSelectionBookIds = await loadClubSelectionBookIds(db, input.userId);
  ...
  for (const evaluation of evaluations) {
    ...
    for (const draft of fresh) {
      const { data } = await db.rpc("record_challenge_contribution", {
  ...
  const badges = await evaluateAndAwardBadges(db, input.userId);
