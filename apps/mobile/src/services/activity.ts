 ...
 * records `activity_events` and formats human-readable feed messages. Uses the
 * shared Supabase client directly instead of passing one in.
 */
...
function formatWithSubject(
  ...
  const percent =
    typeof metadata?.progress_percent === "number" ? metadata.progress_percent : null;
