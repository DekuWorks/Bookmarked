...
async function upsertImportedBook(
  ...
  if (shelfStatus === "read" && previousShelf !== "read") {
    // Do not silently use the import date for a missing completion date:
    // that would distort reading goals and statistics.
    if (!row.dateRead) {
      return {
        title: row.title,
        status: "skipped",
  ...
  const { data: savedUserBook, error: shelfError } = await client
    .from("user_books")
