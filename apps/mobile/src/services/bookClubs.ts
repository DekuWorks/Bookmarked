...
/**
 * Mobile Book Clubs service. Mirrors apps/web/src/lib/services/bookClubs.ts
 * against the Community Hub schema (discussions, invitations, join requests,
 * bookshelf, announcements, club chat, stats). Uses the shared mobile Supabase
 * client and batched hydration to avoid RLS recursion.
 */
...
export async function setMemberNotificationLevel(
  ...
  try {
    ...
    const { error } = await supabase.from("book_club_member_notification_prefs").upsert(
      { club_id: clubId, user_id: user.id, level },
    ...
    return {
      error: err instanceof Error ? err.message : "Could not save notification preference.",
    };
  }
}

/** Search public active clubs by name/description; optional genre filter. */
...
export async function searchClubs(
  viewerId: string,
  query: string,
  limitOrOptions: number | SearchClubsOptions = 30
): Promise<BookClubSummary[]> {
  const options: SearchClubsOptions =
    typeof limitOrOptions === "number" ? { limit: limitOrOptions } : limitOrOptions;
  const limit = options.limit ?? 30;
  const term = query.trim();
  const genres = (options.genres ?? []).map((g) => g.trim()).filter(Boolean);

  if (term.length < 2 && !genres.length) return [];
  ...
  if (genres.length) {
    q = q.overlaps("genre_tags", genres);
  }
 ...
 * club's current book. Mobile-only helper (web uses server-side enrichment).
 */
