...
export async function getReadingActivityData(
  userId: string
): Promise<ReadingActivityData> {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    filled.push(
      byDay.get(key) ?? { day: key, pages_read: 0, session_count: 0 }
    );
  }
...
export async function getReadingHeatmapData(
  userId: string
