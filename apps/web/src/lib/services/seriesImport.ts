...
export function parseSeriesCsv(csv: string): SeriesImportRow[] {
  ...
  return lines.slice(1, 201).map((line) => {
    ...
    return {
      ...
      message: invalid ? "Series name, book title, author, and a numeric position are required." : undefined,
    };
  });
}
...
export async function previewSeriesImport(csv: string): Promise<SeriesImportRow[]> {
  ...
  await Promise.all(rows.map(async (row) => {
    ...
    if (!books?.length) {
      row.status = "book_not_found";
      row.message = "No catalog book matches this title and author.";
      return;
    }
    ...
    const { data: entry } = await supabase
      ...
      .eq("book_id", row.bookId)
      .maybeSingle();
    if (entry) {
      row.status = "duplicate_entry";
      row.message = "This book is already in the existing series.";
    }
  }));
  return rows;
}
  ...
  for (const row of details) {
    ...
    if (!seriesId) {
      const { data: existing, error: findError } = await supabase
        .from("book_series").select("id").ilike("name", row.seriesName).maybeSingle();
      if (findError) {
        row.status = "invalid"; row.message = findError.message; errors += 1; continue;
      }
      if (existing) seriesId = existing.id;
      else {
        const { data: created, error: createError } = await supabase
          .from("book_series").insert({ name: row.seriesName, created_by: userId }).select("id").single();
        if (createError || !created) {
          row.status = "invalid"; row.message = createError?.message ?? "Couldn't create series."; errors += 1; continue;
        }
        seriesId = created.id;
      }
      seriesIds.set(key, seriesId!);
    }
    const { error } = await supabase.from("book_series_entries").insert({
      series_id: seriesId, book_id: row.bookId, position: row.position, entry_type: row.entryType,
    });
    if (error) {
      row.status = "duplicate_entry"; row.message = error.message; errors += 1;
    } else imported += 1;
  }
