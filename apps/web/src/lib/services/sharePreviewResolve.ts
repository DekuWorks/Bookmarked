...
/**
 * Live-resolve a share preview with permission checks.
 * Uses snapshot for display fields; marks unavailable when content is gone,
 * private, or blocked. Dedupes in-flight requests and caches briefly.
 */
...
async function resolveSharePreviewUncached(
  ...
  try {
    ...
    // On resolve failure, keep the snapshot rather than flashing unavailable.
    return snapshotModel;
  }
}
