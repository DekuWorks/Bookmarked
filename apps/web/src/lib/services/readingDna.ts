...
import { parseReadingDnaVisibility, type ReadingDnaPrivacyState } from "@bookmarked/utils/readingDnaPrivacy";
import { DEFAULT_READING_DNA_PRIVACY } from "@bookmarked/utils/readingDnaPrivacy";
import { READING_DNA_VERSION } from "@bookmarked/utils/readingDnaConfig";
...
import { scoreBookDnaMatches, type ReadingDnaBookMatch } from "@bookmarked/utils/readingDnaRecs";
import { compareReadingDnaSnapshots, type ReadingDnaSnapshotCompareRow } from "@bookmarked/utils/readingDnaCompare";
import { cosineReadingDnaMatch, sharesEnoughGenres } from "@bookmarked/utils/readingDnaMatch";
import type { ReadingDnaMatchVector } from "@bookmarked/utils/readingDna";
import { readingDnaStructuredSummary } from "@bookmarked/utils/readingDna";
...
export type ReadingDnaCacheRow = {
  stale: boolean;
...
async function loadCachedProfile(userId: string): Promise<{
  ...
  const { data } = await supabase
    .from("reading_dna_profiles")
    .select(
      "summary, insight, confidence, confidence_score, sample_size, data_points_count, forming, match_vector, dna_version, stale_at, computed_at, visibility, match_enabled, public_top_traits_approved, share_personality_on_reader_map, personality_label, personality_explanation"
    )
    .eq("user_id", userId)
    .maybeSingle();
  ...
  const categories = (["genre", "vibe", "emotion", "trope"] as const).map((category) => ({
    category,
    title:
      category === "genre"
        ? "Genre DNA"
  ...
  const dna: ReadingDna = {
    ...
    topTraits: mapped.filter((trait, index) => traitRows[index]?.is_top_trait),
...
export async function persistReadingDnaSnapshot(
  ...
  const write = async (periodType: "monthly" | "yearly", periodKey: string) =>
...
export async function loadReadingDnaComparisons(userId: string): Promise<{
  ...
  const { data } = await supabase
    ...
    .select("period_type, period_key, payload, created_at")
    .eq("user_id", userId)
  ...
  const yoyPair = yearly.slice(0, 2);
  const momPair = monthly.slice(0, 2);
  ...
  return {
    yoy:
      yoyPair.length === 2 && toDna(yoyPair[1]?.payload) && toDna(yoyPair[0]?.payload)
        ? compareReadingDnaSnapshots(toDna(yoyPair[1]!.payload)!, toDna(yoyPair[0]!.payload)!)
        : [],
    mom:
      momPair.length === 2 && toDna(momPair[1]?.payload) && toDna(momPair[0]?.payload)
        ? compareReadingDnaSnapshots(toDna(momPair[1]!.payload)!, toDna(momPair[0]!.payload)!)
        : [],
  };
}
...
export async function loadDnaBookMatches(userId: string, dna: ReadingDna): Promise<ReadingDnaBookMatch[]> {
  ...
  return scoreBookDnaMatches(dna, candidates).slice(0, 6);
}
