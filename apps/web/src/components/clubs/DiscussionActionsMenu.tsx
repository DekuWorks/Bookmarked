...
import { getClubDiscussionActionPermissions } from "@bookmarked/utils/clubDiscussionUi";
...
/**
 * Discussion ••• menu — Edit only for creator; Delete for creator or host/mod.
 */
export function DiscussionActionsMenu({
  ...
  const name = creatorName?.trim() || "this user";
  ...
  async function handleReport(nextReason: ContentReportReason, nextDetails?: string) {
    ...
    const result = await reportContent({
      contentType: "club_discussion" as ReportableContentType,
  ...
  async function handleBlock() {
    ...
    setBusy(true);
    const result = await blockUser(creatorId, {
      reason: "harassment_bullying",
      details: `Blocked from club_discussion ${discussionId}`,
    });
    setBusy(false);
    setOpen(false);
    if (result.error) {
  ...
  return (
    <div ref={rootRef} className="relative">
      <div className="absolute right-0 z-20 min-w-[160px] rounded-lg border border-border bg-surface py-1 shadow-md">
        {perms.canEdit ? (
          <button
            type="button"
