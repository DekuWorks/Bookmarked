...
export function getShelfSortOptions(shelfStatus?: string): ShelfSortOption[] {
...
export function sortShelfItems<T extends SortableShelfItem>(
  ...
  switch (mode) {
    case "title_asc":
      sorted.sort((a, b) => compareStrings(a.books?.title ?? "", b.books?.title ?? ""));
      break;
    case "title_desc":
      sorted.sort((a, b) => compareStrings(b.books?.title ?? "", a.books?.title ?? ""));
      break;
    case "author_asc":
      sorted.sort((a, b) => {
        const authorCmp = compareStrings(a.books?.author ?? "", b.books?.author ?? "");
    ...
    case "published_oldest":
      ...
      break;
    case "added_oldest":
      sorted.sort((a, b) => parseTimestamp(a.created_at) - parseTimestamp(b.created_at));
      break;
    case "date_to_read":
      sorted.sort((a, b) => {
    ...
    case "progress_updated":
      sorted.sort((a, b) => {
        const updatedCmp = parseTimestamp(b.updated_at) - parseTimestamp(a.updated_at);
    ...
    case "added_newest":
    default:
      sorted.sort((a, b) => parseTimestamp(b.created_at) - parseTimestamp(a.created_at));
      break;
  }
...
export function sortLibraryBookRows<T extends SortableShelfItem>(
  items: T[],
  mode: ShelfSortMode
): T[] {
